...
const UserList: React.FC<UserListProps> = ({ users, selectedUser, onSelectUser }) => {
     const availableUsers = users.filter(u => u.id !== (selectedUser?.id ?? -1))
    return (
...
const ChatWindow: React.FC<ChatWindowProps> = ({ currentUser, selectedUser, messages, users, onSendMessage, onBack }) => {
            ...
            {selectedUser ? (
                <>
                    <div className="flex items-center p-4 border-b border-white/10 bg-base-200/30">
                        <button onClick={onBack} className="md:hidden mr-3 p-1 rounded-full hover:bg-white/10">
                    ...
                    <div className="flex-1 p-6 overflow-y-auto space-y-6">
                        {messages.map(msg => (
                            <div key={msg.id} className={`flex items-end gap-3 ${msg.userId === currentUser.id ? 'justify-end animate-slide-in-right' : 'justify-start animate-slide-in-left'}`}>
...
const ChatView: React.FC = () => {
    const { users, currentUser, conversations, sendDirectMessage, clearChatNotifications } = useApp();
    const [selectedUser, setSelectedUser] = useState<User | null>(null);
    
    if (!currentUser) return null;

    useEffect(() => {
        // Clear notifications when the chat view is opened
        clearChatNotifications(currentUser.id);
    }, [clearChatNotifications, currentUser.id]);
    ...
    const handleSendMessage = (text: string) => {
        if (currentUser && selectedUser) {
            sendDirectMessage(currentUser.id, selectedUser.id, text);
        }
    };
    ...
    return (
        <div className="bg-base-200 border border-white/10 rounded-2xl shadow-2xl h-[calc(100vh-150px)] flex overflow-hidden">
           <UserList users={availableUsers} selectedUser={selectedUser} onSelectUser={setSelectedUser} currentUserId={currentUser.id} />
           <ChatWindow 
