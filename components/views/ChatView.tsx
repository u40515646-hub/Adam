...
const ChatView: React.FC = () => {
    ...
    return (
        <div className="flex h-[calc(100vh_-_10rem)] bg-base-200/50 border border-white/10 rounded-2xl overflow-hidden shadow-2xl animate-fade-in">
            {/* User List */}
            ...
            <div className="w-2/3 flex-col hidden md:flex">
                {selectedUser ? (
                    <>
                        ...
                        <div className="flex-1 p-6 overflow-y-auto space-y-6">
                           {currentMessages.map(msg => (
                                <div key={msg.id} className={`flex items-end gap-3 animate-pop-in ${msg.userId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    <div className={`max-w-sm lg:max-w-lg p-3 px-4 rounded-2xl shadow-md ${msg.userId === currentUser.id ? 'bg-gradient-to-br from-primary to-secondary text-white rounded-br-lg' : 'bg-white/10 text-gray-200 rounded-bl-lg'}`}>
                        ...
                        <div className="p-4 border-t border-white/10 bg-base-200/50">
                            <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                                ...
                                />
                                <button type="submit" className="bg-primary p-3 rounded-full text-white hover:bg-primary-focus transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={!message.trim()}>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                        <Icon name="chat" className="w-16 h-16 text-gray-600 mb-4" />
